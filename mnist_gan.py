...
#class Generator(nn.Module):
 #   def __init__(self, g_input_dim, g_output_dim):
  #      super(Generator, self).__init__()
   #     self.fc1 = nn.Linear(g_input_dim, 100)
     ...
     #   x = torch.tanh(self.fc4(x))
      #  x = F.leaky_relu(self.fc5(x), 0.2)
       # x = F.leaky_relu(self.fc6(x), 0.2)
        #return F.leaky_relu(self.fc7(x), 0.2)
...
for epoch in range(1, n_epoch+1):           
    ...
    if ((epoch%10) == 0):
       with torch.no_grad():
           ...
           generated = G(test_z)
#           generated = G.get_image(test_z)
           save_image(generated.view(generated.size(0), 1, 28, 28), './Figs/gan_orig.png')
