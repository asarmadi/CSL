...
class generator(nn.Module):
    ...
    def __init__(self,in_channels):
        ...
        self.t1=nn.Sequential(
            nn.ConvTranspose2d(in_channels=100,out_channels=512,kernel_size=(4,4),stride=1,padding=0),
            nn.BatchNorm2d(512),
            nn.ReLU()
        )
...
class discriminator(nn.Module):
    ...
    def __init__(self,classes=10):
        ...
        self.fc_source=nn.Linear(4*4*64,1)
        self.fc_class=nn.Linear(4*4*64,classes)
        self.sig=nn.Sigmoid()
