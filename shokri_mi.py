...
def per_label_attack_performance(x_member, y_member, x_non_member, y_non_member, extractor, classifier, args, all=False):
    ...
    if args.pct == 0:
       ...
       indices = np.arange(len(x_member[0])+len(x_member[1]))
    ...
    if args.pct == 0:
       ...
       indices = np.arange(len(x_non_member[0])+len(x_non_member[1]))
    ...
    if args.pct == 0:
       ...
       x_member = torch.cat((x_member[0], x_member[1]))
    x_member = x_member[member_indices]
    if args.pct == 0:
       y_member = y_member[0]
    else:
       y_member = torch.cat((y_member[0], y_member[1]))
    y_member = y_member[member_indices]
   
    if args.pct == 0: 
       x_non_member = x_non_member[0]
    else:
       x_non_member = torch.cat((x_non_member[0], x_non_member[1]))
    x_non_member = x_non_member[non_member_indices]
    if args.pct == 0:
       y_non_member = y_non_member[0]
    else:
       y_non_member = torch.cat((y_non_member[0], y_non_member[1]))
    y_non_member = y_non_member[non_member_indices]
    
    n_samples = 12800
    indices = np.arange(len(x_member))
    mindices = np.random.choice(indices, n_samples, replace=False)
    indices = np.arange(len(x_non_member))
    nindices = np.random.choice(indices, n_samples, replace=False)
    ...
    for label in desired_labels:
        ...
        for j in tqdm(range(0, n_datapoints, args.batch_size)):
            ...
            outs = model(yp)
            outs = (outs >= t).float() * 1
            update_acc_for_batch(np.arange(len(targets)), len(targets), outs, targets.unsqueeze(1), acc, labels_elems)
        if not all:
...
#            member_preds.extend(outs[targets==1])
#            non_member_preds.extend(outs[targets==0])
            outs = (outs >= t).float() * 1
...
#    plt.plot(fpr_sfe, tpr_sfe, 'b', label='SFE')
...
#    plt.plot(fpr_ctfe, tpr_ctfe, 'g', label='CTFE')
 ...
 #   plt.plot(fpr_ltfe, tpr_ltfe, 'c', label='LTFE')
