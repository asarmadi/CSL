...
#        scheduler.step()
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        data, target = data.to(device), target.to(device)
        features = model.get_features(data).cpu().data
        total_features.append(features)
        total_labels.append(target.cpu().data)

    torch.save(total_features, 'checkpoint/X_orig.pt')
    torch.save(total_labels, 'checkpoint/Y_orig.pt')
